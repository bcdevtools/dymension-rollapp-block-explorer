...
export default function ProposalStatusText({ status }: Readonly<{ status: string }>) {
  ...
  return (
    <Typography variant="subtitle2" color={statusColor}>
      ...
      {PROPOSAL_STATUS_DISPLAY[status as keyof typeof PROPOSAL_STATUS_DISPLAY].toUpperCase()}
    </Typography>
  );
}
